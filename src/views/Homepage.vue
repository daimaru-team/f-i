        ...
        <v-toolbar class="v-toolbar v-toolbar--fixed theme--dark red elevation-6"
         ...
         style="margin-top:0px;padding-right:0px;padding-left:0px;transform:translateY(0px);">
            ...
            <v-layout v-else-if="window.width < 1200 && window.width > 500">
                ...
                <v-flex xs4 sm4 md4 headline font-weight-black font-italic
                class="text-xs-right"><span><digital-clock :blink="true"/>
                </span></v-flex>
            </v-layout>
        ...
        <v-navigation-drawer v-model="drawer"
         class="grey darken-4"
          dark fixed hide-overlay width="250">
            ...
            <v-list class="pt-0">
                ...
                :name="item.name">
                    <v-flex>
                        <h3>
                        <v-icon size="23" style="padding:0px 4px 0px 0px">
                          {{item.icon}}
                          </v-icon>
                            {{item.title}}
                        </h3>
                  ...
                  <v-btn v-if="window.width < 1200"
                   color="yellow" class="black--text"
                   bottom right small fab fixed style="border-radius: 60px 0px 0px 60px"
          ...
          <main class="v-content" style="padding: 47px 0px 0px 0px;" width="100%">
              <v-layout v-if="window.width > 1200"
                style="padding: 0px 0px 0px 251px;"
                justify-start>
                  <router-view />
              </v-layout>
              <v-layout v-if="window.width < 1200"
            ...
            <v-card>
                <v-list class="grey darken-4 white--text">
                    <v-list-tile avatar>
                        ...
                        <v-list-tile-content>
                            <v-list-tile-title><b>
                              {{Display_user_nav.fullname}} </b>
                              ({{Display_user_nav.nickname}})</v-list-tile-title>
                            <v-list-tile-sub-title class="white--text">
                              สิทธิ์การเข้าถึง :{{Store.User_type_store}}
                ...
                <v-card-text class="grey lighten-3">
                    ...
                    <!-- <v-list-tile-title  v-for="(item,i) in items_Cars"
                    :key="i">Car {{i+1}}:{{items_Cars}}</v-list-tile-title> -->
                </v-card-text>
                <v-divider></v-divider>

                <v-card-actions class="grey lighten-3">
                    <v-spacer></v-spacer>
                    <v-btn color="red" style="border-radius:40px 0px 40px 0px"
                    @click="dialog_Morword = false">OK</v-btn>
                </v-card-actions>
